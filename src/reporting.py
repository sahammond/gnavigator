...
def report_cDNA(prefix, cDNA_results, TOT):
    ...
    pct_sum_complete = round(pct_complete + pct_duplicated, 2)
    ...
    with open(tsvout, 'w') as outfile:
        ...
        nums = '\t'.join([str(x) for x in ['Number', sum_complete, num_complete,
                                           num_duplicated, num_fragmented, num_partial,
                                           num_poor, num_missing, num_counted]])
        pcts = '\t'.join([str(x) for x in ['Percent', pct_sum_complete, pct_complete,
                                           pct_duplicated, pct_fragmented, pct_partial,
                                           pct_poor, pct_missing, pct_counted]])
        print(util.report_cmd(), file=outfile)
        print(header, file=outfile)
        print(nums, file=outfile)
        print(pcts, file=outfile)
    jiraout = f'{prefix}-results.jira'
    with open(jiraout, 'w') as outfile:
        header_txt.extend(['']) # add final empty string for jira table separator
        header = '||'.join(header_txt)
        nums = [sum_complete, num_complete, num_duplicated, num_fragmented, num_partial,
                num_poor, num_missing, num_counted]
        pcts = [pct_sum_complete, pct_complete, pct_duplicated, pct_fragmented,
    ...
    print(f'{sum_complete} ({pct_sum_complete}%) complete sequences')
    print(f'  {num_complete} ({pct_complete}%) complete, single copy sequences')
    print(f'  {num_duplicated} ({pct_duplicated}%) complete, multiple copy sequences')
    print(f'{num_fragmented} ({pct_fragmented}%) fragmented sequences')
    print(f'{num_partial} ({pct_partial}%) partial sequences')
    print(f'{num_poor} ({pct_poor}%) poorly mapped sequences')
    print(f'{num_missing} ({pct_missing}%) missing sequences')
    print(f'{num_counted} ({pct_counted}%) sequences were evaluated')
    print(util.report_time())


def output_cDNA(prefix, cDNA_results, gm=''):
    # write out cDNA:scaffold mappings
    full_out = f'{prefix}-full-cDNA-results-table.tsv'
...
def output_gm(prefix, gm_res):
    ...
    full_out = f'{prefix}-full-genetic-map-results-table.tsv'
    with open(full_out, 'w') as outfile:
        print(util.report_cmd(), file=outfile)
        print(header, file=outfile)
        # TODO what do I do with status here? Or is it just discarded because there
        #  should only be complete, single cDNAs in gm_res
...
def report_gm(uniqDatMap_select, gm_results, gm_cdna_statuses, prefix):
    ...
    if num_scaff_toCheck == num_scaff_checked:
        ...
        print('Not all scaffolds to be checked against genetic map were successfully '
            'checked.')
    ...
    # write to tsv
    tsvout = f'{prefix}-genetic-map-scaffold-results.tsv'
    with open(tsvout, 'w') as outfile:
        header_txt = ['', '1 complete cDNA', '2+ complete cDNAs',
                      'Same LG, expected order', 'Same LG, unexpected order',
                      'Different LG', 'Same LG, undetermined order']
        header = '\t'.join(header_txt)
        nums = '\t'.join([str(x) for x in ['Number', num_solo, num_2plus_scaff,
                                           num_goodLG, num_WO_LG, num_diffLG,
                                           num_undet]])
        ...
        print(nums, file=outfile)
        print(pcts, file=outfile)
    jiraout = f'{prefix}-genetic-map-scaffold-results.jira'
    with open(jiraout, 'w') as outfile:
        header_txt.extend(['']) # add final empty string for jira table separator
        header = '||'.join(header_txt)
    ...
    print(f'{num_solo} ({pct_solo}%) had exactly 1 complete cDNA from the genetic map ',
        'aligned to them.')
    print(f'{num_2plus_scaff} ({pct_LGscaff}%) had 2+ complete cDNAs from the genetic ',
        'map aligned to them.')
    print(f'  {num_goodLG} ({pct_goodLG}%) were from the same linkage group and in the ',
        'expected order.')
    print(f'  {num_WO_LG} ({pct_WO_LG}%) were from the same linkage group, but NOT in ',
        'the expected order.')
    print(f'  {num_diffLG} ({pct_diffLG}%) were from different linkage groups.')
    print(f'  {num_undet} ({pct_undet}%) were from the same linkage group but their ',
...
def report_gm_cDNA(gm_results, uniqMapDat_select, prefix):
    ...
    # write to tsv
    tsvout = f'{prefix}-genetic-map-cDNA-results.tsv'
    with open(tsvout, 'w') as outfile:
        ...
        nums = '\t'.join([str(x) for x in ['Number', num_solo, num_good, num_wo, num_diff,
                                           num_undet]])
        pcts = '\t'.join([str(x) for x in ['Percent', pct_solo, pct_good, pct_wo,
                                           pct_diff, pct_undet]])
        print(util.report_cmd(), file=outfile)
        print(header, file=outfile)
        print(nums, file=outfile)
        print(pcts, file=outfile)
    jiraout = f'{prefix}-genetic-map-cDNA-results.jira'
    with open(jiraout, 'w') as outfile:
        header_txt.extend(['']) # add final empty string for jira table separator
        header = '||'.join(header_txt)
    ...
    print(f'{num_solo} ({pct_solo}%) were found alone on a scaffold and were not ',
        'evaluated further.')
    print(f'{num_good} ({pct_good}%) were from the same linkage group and were found on ',
        'scaffolds in the expected order')
    print(f'{num_wo} ({pct_wo}%) were from the same linkage group but were found on ',
        'scaffolds in an unexpected order.')
    print(f'{num_diff} ({pct_diff}%) were from different linkage groups but were found ',
        'on the same scaffold.')
    print(f'{num_undet} ({pct_undet}%) were from the same linkage group but their order ',
